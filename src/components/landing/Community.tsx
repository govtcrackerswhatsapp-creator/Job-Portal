  ...
  return (
    <section id="community" className="max-w-6xl mx-auto px-4 py-20 scroll-mt-24">
      <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
        <div className="lg:col-span-7 relative flex items-center justify-center p-6 sm:p-12 rounded-3xl min-h-[340px] sm:min-h-[380px] overflow-hidden bg-gradient-to-b from-white to-[#f5f5f7] border border-black/5 shadow-soft">
          {loading ? <Loader2 className="w-8 h-8 text-[#8b2df2] animate-spin" /> : selected ? (
            <>
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <a href={selected.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 max-w-full whitespace-nowrap px-6 py-3 rounded-full text-white text-sm font-bold shadow-md hover:scale-[1.03] transition-all" style={{ backgroundColor: accent }}>Visit {selected.platform} <ExternalLink className="w-4 h-4" /></a>
              </div>
            </>
          ) : null}
        </div>
        <div className="lg:col-span-5 flex flex-col justify-center">
          ...
          {selected ? <div className="mt-8 text-sm font-semibold text-zinc-800 bg-zinc-100/80 border border-zinc-200 px-4 py-2.5 rounded-xl inline-flex flex-wrap items-center gap-2 self-start max-w-full"><Share2 className="w-4 h-4" style={{ color: accent }} /> Currently viewing: <span className="uppercase tracking-wider" style={{ color: accent }}>{selected.platform}</span> <ArrowUpRight className="w-4 h-4 text-zinc-400" /></div> : null}
        </div>
      </div>
    </section>
  );
