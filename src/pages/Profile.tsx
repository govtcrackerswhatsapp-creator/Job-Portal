    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-soft p-6 mb-4">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#8b2df2] to-[#00b4d8] flex items-center justify-center text-white text-2xl font-bold">
            {(user.name || user.email).charAt(0).toUpperCase()}
          </div>
          <div>
            <h2 className="font-heading text-xl font-bold text-zinc-900">{user.name || 'User'}</h2>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs text-zinc-400">Email</p><p className="text-sm font-medium text-zinc-900">{user.email}</p></div>
          </div>
          <div className="flex items-center gap-3">
            <Shield className="w-4 h-4 text-zinc-400" />
            <div><p className="text-xs text-zinc-400">Role</p><p className="text-sm font-medium text-zinc-900 capitalize">{user.role}</p></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-soft p-6">
        ...
        <div className={`flex items-center gap-3 p-4 rounded-xl ${hasAccess ? 'bg-emerald-50' : 'bg-zinc-50'}`}>
          {hasAccess ? <CheckCircle2 className="w-6 h-6 text-emerald-600" /> : <XCircle className="w-6 h-6 text-zinc-400" />}
          <div>
